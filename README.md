...
- Supabase reward backend scaffolding for future cross-device idempotency, including reward-event migrations, an atomic award RPC, and a feature-gated frontend service wrapper that preserves local fallback behavior.
...
- Backend reward-event scaffolding exists, but it is disabled by default until the Supabase migrations are applied and authenticated reward flows are validated in a real project.
- The local reward-event ledger remains the default fallback; durable retry/reconciliation, local import/backfill, broader data-model hardening, and accessibility regression coverage are still planned work.
