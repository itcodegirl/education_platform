...
export function AppLayout() {
  ...
  const {
    completed, completedSet, toggleLesson,
    awardXP, recordDailyActivity,
    savePosition, trackCourseVisit,
    dataLoaded, lastPosition,
  } = useProgress();
  ...
  const readTime = useMemo(() => estimateReadingTime(les.content + les.code), [les.content, les.code]);
  ...
  }, [lessonKey, isDone, marking, toggleLesson, awardXP, recordDailyActivity]);
  ...
  // ─── Keyboard ─────────────────────────────
  useKeyboardNav({
    onNext: nav.next, onPrev: nav.prev,
    ...
    <div className={`shell ${theme}`} data-course={course.id}>
      ...
      <main className="mn" ref={mainRef}>
        <div className="topbar">
          ...
          {!showModQuiz && (
            <button type="button" className={`mark-btn ${isDone ? 'dn' : ''}`} onClick={handleMarkDone} disabled={marking}>
        ...
        <div className="lv-wrap">
          {showModQuiz && moduleQuiz ? (
            <div className="lv">
              <div className="lv-head"><span className="lv-emoji">📝</span><h2 className="lv-title">{mod.title} — Module Quiz</h2></div>
            ...
            <>
              <LessonView lesson={les} emoji={mod.emoji} lang={course.id} lessonKey={lessonKey} courseId={course.id} moduleTitle={mod.title} />
              {lessonQuiz && (
      ...
      <XPPopup />
      <BadgeUnlock />
      <PanelManager panels={panels} nav={nav} course={course} profile={profile} completed={completed} lastPosition={lastPosition} courseTotal={courseTotal} />
    </div>
  );
}
