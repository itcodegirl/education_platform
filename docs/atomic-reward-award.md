...
The function never accepts `user_id` from the client. It derives the learner from `auth.uid()`, derives the canonical event key from the event type, entity id, and authenticated user id, and derives XP from `public.reward_catalog`.
...
- Reject stale or forged event keys when an older client sends one.
- Reject unsupported reward event types.
- Reject missing entity IDs.
- Reject unknown or malformed reward entities.
- Reject stale or forged XP amounts when an older client sends one.
