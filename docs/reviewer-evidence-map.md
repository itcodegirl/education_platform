...
| Progress evidence separates retention, remediation, and XP. | `src/components/panels/StudentStats.jsx`, `src/utils/retentionSignals.js`, `src/utils/moduleMasteryEvidence.js` | `src/utils/retentionSignals.test.js`, `src/utils/moduleMasteryEvidence.test.js`, and `src/components/panels/StudentStats.test.jsx`. |
| Challenge completion has a visible definition of done, not just a motivational badge. | `src/components/panels/ChallengesPanel.jsx`, `src/utils/challengeEvidence.js`, `src/styles/feature-challenges.css` | `src/utils/challengeEvidence.test.js` and `src/components/panels/ChallengesPanel.test.jsx` cover evidence scope, rubric copy, and trust boundaries. |
...
| Visual assets have explicit page-weight and preload rules. | `docs/asset-performance-policy.md`, `scripts/check-asset-performance-policy.mjs`, `scripts/check-asset-sizes.mjs`, `docs/performance-evidence.md` | `npm run audit:asset-policy` verifies image, font, media, and preload guidance; `npm run audit:asset-sizes` checks committed asset files before visual additions become performance debt. |
| Production performance can be observed without adding a new vendor dependency. | `src/lib/webVitals.js`, `src/App.jsx`, `src/lib/analytics.js`, `docs/performance-evidence.md` | Unit tests verify low-cardinality Web Vitals payloads; analytics remains optional and no-op unless configured. |
