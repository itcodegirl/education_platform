...
export async function fetchAllUserData(uid) {
  const [
    progressRes, quizRes, xpRes, streakRes, dailyRes,
    badgesRes, srRes, bookmarksRes, notesRes, visitedRes, posRes,
  ] = await Promise.all([
...
export function addLesson(uid, lessonKey) {
  return supabase.from('progress').upsert({ user_id: uid, lesson_key: lessonKey });
}

export function removeLesson(uid, lessonKey) {
  return supabase.from('progress').delete().eq('user_id', uid).eq('lesson_key', lessonKey);
}

// ─── Quiz Scores ────────────────────────────
export function saveQuizScore(uid, quizKey, score) {
  return supabase.from('quiz_scores').upsert({
    user_id: uid, quiz_key: quizKey, score, completed_at: new Date().toISOString(),
  });
}
...
export function updateXP(uid, total) {
  return supabase.from('xp').upsert({
    user_id: uid, total, updated_at: new Date().toISOString(),
  });
}
...
export function updateStreak(uid, days, lastDate) {
  return supabase.from('streaks').upsert({
    user_id: uid, days, last_date: lastDate, updated_at: new Date().toISOString(),
  });
}
...
export function updateDailyGoal(uid, goalDate, count) {
  return supabase.from('daily_goals').upsert({
    user_id: uid, goal_date: goalDate, count, updated_at: new Date().toISOString(),
  });
}
...
export function awardBadge(uid, badgeId) {
  return supabase.from('badges').upsert({
    user_id: uid, badge_id: badgeId, earned_at: new Date().toISOString(),
  });
}
...
export function updateSRCard(uid, question, updates) {
  return supabase.from('sr_cards').update({
    ...updates,
  }).eq('user_id', uid).eq('question', question);
}
...
export function removeBookmark(uid, lessonKey) {
  return supabase.from('bookmarks').delete().eq('user_id', uid).eq('lesson_key', lessonKey);
}

// ─── Notes ──────────────────────────────────
export function saveNote(uid, lessonKey, content) {
  return supabase.from('notes').upsert({
    user_id: uid, lesson_key: lessonKey, content, updated_at: new Date().toISOString(),
  });
}
...
export function trackCourseVisit(uid, courseId) {
  return supabase.from('courses_visited').upsert({
    user_id: uid, course_id: courseId, visited_at: new Date().toISOString(),
  });
}
