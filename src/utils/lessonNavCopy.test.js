...
describe('getNextStepHint', () => {
  it('matches the four user-facing states', () => {
    ...
    expect(getNextStepHint({ isLast: false, showModQuiz: true, isDone: false })).toMatch(/best score saves/);
    expect(getNextStepHint({ isLast: false, showModQuiz: false, isDone: false })).toMatch(/Mark done/);
    expect(getNextStepHint({ isLast: false, showModQuiz: false, isDone: true })).toMatch(/Progress is saved/);
  });
});
