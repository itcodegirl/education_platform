...
{ lessonId:'h1-1', questions:[
  ...
  { id:'h1c', type:'mc', question:'In a real project, what should HTML describe first?', options:['Visual design','Button click behavior','Structure and content','Server login logic'], correct:2, explanation:'HTML should describe the page structure and content first. CSS handles design, JavaScript handles behavior, and backend code handles server logic.', optionFeedback:['Design matters, but color, spacing, and layout belong to CSS after the content structure is clear.','Button behavior is JavaScript work. HTML can create the button, but it should not own the interaction logic.','','Server login logic belongs on the backend. HTML can show a form, but it does not authenticate users.'] },
  { id:'h1d', type:'mc', question:'Why is HTML the frame and structure in the house analogy?', options:['Paint and wallpaper','Electricity','Frame and structure','Furniture'], correct:2, explanation:'Because HTML gives content its structural frame before CSS adds visual treatment or JavaScript adds behavior.' },
  ...
  { id:'h1g', type:'mc', question:'Which is NOT something HTML can do?', options:['Create headings','Make a link','Calculate 2+2','Display an image'], correct:2, explanation:'Calculations require JavaScript. A common mistake is expecting HTML to compute values when it only describes content.' },
]},
...
{ lessonId:'h1-2', questions:[
  ...
  { id:'h2d', type:'mc', question:'In a real project, why should JavaScript change the DOM instead of the HTML file on disk?', options:['The CSS file is always edited first','The DOM is the live page the browser is showing','The server changes HTML automatically','The saved HTML file updates after every click'], correct:1, explanation:'JavaScript works with the live DOM because that is what users see. A common mistake is expecting browser JavaScript to rewrite the saved HTML source file.', optionFeedback:['CSS can style elements, but JavaScript still needs the live DOM to find and change page content.','','Servers can send new HTML, but browser JavaScript usually changes the current DOM after the page loads.','Clicks can update the live page, but they do not rewrite the original HTML file saved in your project.'] },
