...
describe('supabase policy sql static checks', () => {
  ...
  it('reward-backend.enforces-idempotency-and-auth-owned-awards statically', () => {
    ...
    const trustBoundaryMigration = readText('../../supabase/migrations/202605110001_harden_reward_event_trust_boundaries.sql');
    ...
    [awardRpcMigration, trustBoundaryMigration].forEach((sql) => {
    ...
    expect(trustBoundaryMigration).toMatch(/create table if not exists public\.reward_catalog/i);
    expect(trustBoundaryMigration).toMatch(/alter table public\.reward_catalog enable row level security/i);
    expect(trustBoundaryMigration).toMatch(/grant select on table public\.reward_catalog to authenticated/i);
    expect(trustBoundaryMigration).toMatch(/v_event_key := concat\(v_catalog\.event_key_prefix/i);
    expect(trustBoundaryMigration).toMatch(/v_catalog\.xp_amount/i);
    expect(trustBoundaryMigration).toMatch(/event_key_mismatch/i);
    expect(trustBoundaryMigration).toMatch(/xp_amount_mismatch/i);
  });
