...
- Streak count: the persisted streak in the DB is the value as of the learner's most recent activity. The UI applies an active-streak guard (`getActiveStreakDays` in `src/utils/helpers.js`) so a learner who has missed more than one day sees `0` instead of the stale value. The DB write path is unchanged; the next activity inside the today/yesterday window resumes the saved count cleanly. Companion `getPausedStreak` surfaces the lapsed value as a positive recovery cue (used by both the WelcomeBack overlay and the topbar pill).
...
- Challenge auto-grading is a mix of source-text checks and DOM-based checks. Two HTML challenges have been migrated so far — `html-ch-1` ("Build a Navigation Bar") and `html-ch-3` ("Semantic Page Layout") — and use live `iframe.contentDocument` queries. A learner can no longer pass "uses `<nav>`" or "no `<div>` elements" by typing the substring inside an HTML comment, because the parser strips comments before `querySelector` runs. The remaining challenges in `src/data/{html,css,js}/challenges.js` still use the older `string.includes` / regex helpers. The infrastructure (`useChallengeSession.runTests` awaits the iframe's `onLoad` with a 1.5s safety timeout, and the test signature accepts `(code, iframe)`) is in place; migrating the rest is purely data — change each `check:(c)=>...` to `check:(_, iframe)=>...` using the `dom(iframe)` helper at the top of `html/challenges.js`. The CodeChallenge UI still states this distinction explicitly under the test results so the limitation stays visible to learners.
