...
- Lessons render through two paths: the structured `StructuredLessonBody` (`hook`/`do`/`understand`/`build`/`challenge`/`summary`/`bridge`) and the legacy `RichLessonBody` (`concepts`/`code`/`output`/`tasks`/`challenge`/`devFession`). HTML, CSS, and JS lessons are on the structured shape; the React course (41 lessons) is still on the legacy shape pending a curriculum-led rewrite. `npm run audit:lesson-format` (wired into `npm run check:quality`) locks the direction: new lessons must be structured, and the legacy React lessons are an explicit, shrinking allowlist in `scripts/lesson-format-allowlist.json`. See [docs/handoff-deferred-risks.md](./docs/handoff-deferred-risks.md) Risk A for the migration plan.
